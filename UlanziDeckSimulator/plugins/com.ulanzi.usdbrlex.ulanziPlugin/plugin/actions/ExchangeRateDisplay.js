...
function ExchangeRateDisplay(ctx) {
     ...
     */
    function initialize() {
        // Create 72x72px canvas for button display
    ...
    function renderLoadingState() {
        ...
        // Clear canvas with UlanziDeck standard background
        canvasContext.fillStyle = '#282828';
        canvasContext.fillRect(0, 0, 72, 72);
        
        // Draw currency pair label to show what we're loading
        canvasContext.fillStyle = '#CCCCCC';
        canvasContext.font = '8px Source Han Sans SC, sans-serif';
        canvasContext.fillText('USD/BRL', 36, 15);
        
        // Draw loading indicator
        canvasContext.fillStyle = '#FFFFFF';
        canvasContext.font = '10px Source Han Sans SC, sans-serif';
        canvasContext.fillText('Loading...', 36, 32);
        
        // Show connectivity status if offline
        if (!CurrencyAPI.getConnectivityStatus()) {
            canvasContext.fillStyle = '#FFAA00';
            canvasContext.font = '7px Source Han Sans SC, sans-serif';
            canvasContext.fillText('Waiting for', 36, 48);
            canvasContext.fillText('connection', 36, 58);
        } else {
            // Show retry count if this is a retry attempt
            if (retryCount > 0) {
                canvasContext.fillStyle = '#CCCCCC';
                canvasContext.font = '7px Source Han Sans SC, sans-serif';
                canvasContext.fillText(`Retry ${retryCount}`, 36, 48);
            }
        }
    ...
    function renderRate(rate, timestamp) {
        ...
        const isOffline = !CurrencyAPI.getConnectivityStatus();
    ...
    function renderValidRateDisplay(rate, timestamp, isStale, isOffline, primaryColor, secondaryColor, accentColor) {
        ...
        canvasContext.font = '8px Source Han Sans SC, sans-serif';
        canvasContext.fillText('USD/BRL', 36, 12);
        
        // Draw exchange rate with appropriate color
        canvasContext.fillStyle = primaryColor;
        canvasContext.font = 'bold 14px Source Han Sans SC, sans-serif';
        canvasContext.fillText(rate.toFixed(4), 36, 32);
        ...
        if (timestamp) {
            canvasContext.fillStyle = secondaryColor;
            canvasContext.font = '6px Source Han Sans SC, sans-serif';
            const timeStr = timestamp.toLocaleTimeString('en-US', { 
                hour12: false, 
                hour: '2-digit', 
                minute: '2-digit' 
            });
            canvasContext.fillText(timeStr, 36, statusY);
            statusY += 8;
        }
        ...
        if (isOffline) {
            ...
            canvasContext.font = '6px Source Han Sans SC, sans-serif';
            canvasContext.fillText('OFFLINE', 36, statusY);
            statusY += 8;
        } else if (isStale) {
            canvasContext.fillStyle = accentColor;
            canvasContext.font = '6px Source Han Sans SC, sans-serif';
            canvasContext.fillText('STALE DATA', 36, statusY);
            statusY += 8;
        }
        
        // Show retry indicator if we've had recent failures
        if (retryCount > 0 && errorState) {
            canvasContext.fillStyle = accentColor;
            canvasContext.font = '5px Source Han Sans SC, sans-serif';
            canvasContext.fillText(`${retryCount} retries`, 36, statusY);
        }
    }
    ...
    function renderErrorStateDisplay(isOffline, primaryColor, secondaryColor, accentColor) {
        ...
        // Draw primary error message
        canvasContext.fillStyle = primaryColor;
        canvasContext.font = 'bold 10px Source Han Sans SC, sans-serif';
        canvasContext.fillText(errorMessage, 36, 36);
        
        // Draw detail message
        if (detailMessage) {
            canvasContext.fillStyle = secondaryColor;
            canvasContext.font = '7px Source Han Sans SC, sans-serif';
            canvasContext.fillText(detailMessage, 36, 48);
        }
        ...
        if (retryCount > 0) {
            canvasContext.fillStyle = secondaryColor;
            canvasContext.font = '6px Source Han Sans SC, sans-serif';
            canvasContext.fillText(`Attempt ${retryCount}/${CurrencyAPI.MAX_RETRIES + 1}`, 36, 58);
        }
        
        // Show fallback instructions
        canvasContext.fillStyle = secondaryColor;
        canvasContext.font = '5px Source Han Sans SC, sans-serif';
        canvasContext.fillText('Check connection', 36, 68);
    }
    ...
    function setRefreshInterval(minutes) {
        const newInterval = parseInt(minutes) || 5;
    ...
    function triggerRefresh() {
        ...
        // Call the enhanced API with connectivity awareness
        CurrencyAPI.getUSDToBRL(true).then(response => {
            if (response.success && response.rate !== null && !isNaN(response.rate)) {
                ...
                // If we were in degraded mode, exit it
            ...
            const errorMessage = CurrencyAPI.handleAPIErrorWithConnectivity(error);
            updateRate(null, new Date(), true, errorMessage);
        });
    }
